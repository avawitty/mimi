...
export const DeveloperSettings: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  return (
    ...
    >
      <div className="w-full max-w-2xl bg-stone-900 border border-stone-800 rounded-sm shadow-2xl p-8 md:p-12 space-y-10 flex flex-col max-h-[85vh]">
            ...
            <AnimatePresence mode="wait">
                {activeTab === 'config' ? (
                    <motion.div key="config" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8">
                        <div className="flex items-center justify-between p-4 bg-black/40 border border-white/5 rounded-sm">
                            ...
                            <button 
                                ...
                                className={`w-12 h-6 rounded-full transition-colors relative ${agentConfig.curatorEnabled ? 'bg-emerald-600' : 'bg-stone-800'}`}
                            >
                                <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${agentConfig.curatorEnabled ? 'left-7' : 'left-1'}`} />
                            </button>
                        </div>
                        ...
                        <div className="flex items-center justify-between p-4 bg-black/40 border border-white/5 rounded-sm">
                            ...
                            <button 
                                ...
                                className={`w-12 h-6 rounded-full transition-colors relative ${agentConfig.sentinelEnabled ? 'bg-emerald-600' : 'bg-stone-800'}`}
                            >
                                <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${agentConfig.sentinelEnabled ? 'left-7' : 'left-1'}`} />
                            </button>
                        </div>
                    ...
                    <motion.div key="console" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col h-full space-y-6">
                        <div className="flex-1 bg-black/60 border border-stone-800 p-4 rounded-sm font-mono text-[10px] text-stone-300 overflow-y-auto min-h-[300px] space-y-2">
                        ...
                        <div className="flex gap-4">
                            <button 
                                ...
                                className="flex-1 py-3 bg-stone-800 hover:bg-stone-700 text-white rounded-sm font-sans text-[9px] uppercase tracking-widest font-black flex items-center justify-center gap-2 transition-all disabled:opacity-50"
                            >
