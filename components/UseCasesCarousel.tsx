...
export const UseCasesCarousel: React.FC = () => {
  ...
  return (
    <div className="w-full bg-white dark:bg-stone-900 border border-stone-100 dark:border-stone-800 rounded-2xl p-8 md:p-12 relative overflow-hidden shadow-sm">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="flex items-center gap-4">
            <div className="p-3 bg-stone-50 dark:bg-stone-950 rounded-full text-nous-accent">
              {active.icon}
            </div>
            <div>
      ...
      <div className="flex gap-4 mt-12">
        <button onClick={prev} className="p-2 border border-stone-100 dark:border-stone-800 rounded-full text-stone-300 hover:text-nous-text transition-all"><ChevronLeft size={16} /></button>
        <button onClick={next} className="p-2 border border-stone-100 dark:border-stone-800 rounded-full text-stone-300 hover:text-nous-text transition-all"><ChevronRight size={16} /></button>
        <div className="flex-1 flex items-center justify-end gap-2">
          {USE_CASES.map((_, i) => (
            <div key={i} className={`w-1.5 h-1.5 rounded-full transition-all ${i === index ? 'bg-nous-accent w-4' : 'bg-stone-200'}`} />
          ))}
        </div>
      </div>
    </div>
