...
export const EntropyDensityMatrix: React.FC<EntropyDensityMatrixProps> = ({
  ...
  return (
    <div className="flex flex-col items-center w-full">
      <div 
        ref={containerRef}
        className={`relative w-full aspect-square max-w-[300px] bg-stone-50 dark:bg-[#050505] border border-stone-200 dark:border-stone-800 rounded-xl overflow-hidden ${readOnly ? '' : 'cursor-crosshair touch-none'}`}
      ...
      >
        ...
        {userDensity !== undefined && userEntropy !== undefined && (
          <div 
            className="absolute w-16 h-16 -ml-8 -mb-8 rounded-full bg-emerald-500/5 border border-emerald-500/20 pointer-events-none flex items-center justify-center"
        ...
        {/* Artifact Node */}
        <motion.div 
          className="absolute w-4 h-4 -ml-2 -mb-2 rounded-full bg-[var(--hover-accent)] shadow-[0_0_10px_var(--hover-accent)] pointer-events-none z-10"
      ...
      <div className="mt-4 flex flex-col items-center w-full max-w-[300px]">
        ...
        {!readOnly && hasChanged && (
          <button 
            onClick={handleSave}
            className="flex items-center gap-2 px-4 py-2 bg-[var(--hover-accent)] text-white text-[10px] font-bold uppercase tracking-widest rounded-full hover:opacity-90 transition-opacity mt-2"
          >
