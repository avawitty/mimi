...
export const Diagnostics: React.FC = () => {
  ...
  return (
    <div className="p-6 bg-stone-50 dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-sm space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button 
          ...
          disabled={isRunning}
          className="px-6 py-2 bg-nous-text dark:bg-white text-white dark:text-black font-sans text-[9px] uppercase tracking-widest font-black rounded-full shadow-lg active:scale-95 transition-all disabled:opacity-50 flex items-center gap-2"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {results.map(r => (
          <div key={r.id} className="flex items-center gap-4 p-4 border border-stone-100 dark:border-stone-800 rounded-sm bg-white dark:bg-black/20">
            <div className={`p-2 rounded-full ${
              r.status === 'pending' ? 'bg-stone-100 dark:bg-stone-800 text-stone-400' :
              r.status === 'success' ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-500' :
