...
export const SocialShareModal: React.FC<SocialShareModalProps> = ({ metadata, onClose }) => {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(imperialUrl);
    setCopied(true);
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }} 
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[11000] flex items-center justify-center p-6 bg-stone-950/80 backdrop-blur-xl selection:bg-emerald-500 selection:text-white"
    >
      <motion.div 
        initial={{ scale: 0.9, y: 20 }}
        animate={{ scale: 1, y: 0 }}
        exit={{ opacity: 0, scale: 0.9, y: 20 }}
        className="relative w-full max-w-md bg-white dark:bg-stone-900 rounded-sm border border-stone-200 dark:border-stone-800 shadow-2xl overflow-hidden"
      >
        <div className="p-8 md:p-10 space-y-8">
           ...
           <div className="space-y-6">
              {/* THE IMPERIAL LINK */}
              <div className="bg-stone-50 dark:bg-stone-950 p-6 rounded-xl border border-black/5 dark:border-white/5 space-y-4 shadow-inner">
                 <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-black rounded-sm overflow-hidden flex-shrink-0 border border-white/10 shadow-lg">
                       <img 
                 ...
                 <div className="relative group">
                    ...
                    <input 
                      readOnly 
                      value={imperialUrl} 
                      className="w-full bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-lg py-3 pl-10 pr-12 font-mono text-[9px] text-stone-400 focus:outline-none" 
                    />
                    <button 
                        onClick={handleCopy} 
                        className={`absolute right-2 top-1/2 -translate-y-1/2 p-2 transition-all ${copied ? 'text-emerald-500 scale-110' : 'text-stone-300 hover:text-emerald-500'}`}
              ...
              <div className="grid grid-cols-2 gap-4">
                 <button 
                  onClick={shareToX}
                  className="flex flex-col items-center justify-center gap-3 py-6 bg-stone-50 dark:bg-stone-800 border border-stone-100 dark:border-stone-700 rounded-2xl hover:bg-stone-100 dark:hover:bg-stone-700 transition-all group"
                 >
                 ...
                 <button 
                  ...
                  className="flex flex-col items-center justify-center gap-3 py-6 bg-stone-50 dark:bg-stone-800 border border-stone-100 dark:border-stone-700 rounded-2xl hover:bg-stone-100 dark:hover:bg-stone-700 transition-all group"
                 >
                    <Send size={20} className="text-emerald-500 group-hover:scale-110 transition-transform" />
                    <span className="font-sans text-[8px] uppercase tracking-widest font-black">Share Sheet</span>
                 </button>
              </div>

              {/* INSTAGRAM NOTE */}
              <div className="p-5 bg-amber-500/5 rounded-xl border border-amber-500/10 space-y-3">
