...
export const Visualizer: React.FC<{ 
 ...
 {/* VARIANT CONTROLS */}
 <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 bg-black/60 backdrop-blur-2xl p-1.5 rounded-none border border-white/10 z-40 opacity-0 group-hover/visualizer:opacity-100 transition-all">
 <div className="flex bg-white/10 px-3 py-1 rounded-none gap-2 border-r border-white/10 pr-4 mr-1">
 {[0, 1, 2].map((i) => (
 <button 
 key={i} 
 disabled={!variants[i]}
 ...
 <button onClick={saveToPocket} className={`p-2.5 transition-all ${isPocketSaved ? 'text-nous-subtle' : 'text-nous-subtle hover:text-nous-text'}`}>{isPocketSaved ? <Check size={14}/> : <Bookmark size={14}/>}</button>
