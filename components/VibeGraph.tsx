    ...
    <div className="relative w-full h-full bg-stone-50 dark:bg-[#111] rounded-lg border border-stone-200 dark:border-stone-800 overflow-hidden">
      <div className="absolute top-4 right-4 z-10">
        <button
          ...
          className="flex items-center gap-2 bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 px-4 py-2 rounded-full shadow-sm hover:shadow-md transition-all disabled:opacity-50 disabled:cursor-not-allowed"
        >
      ...
      {selectedNode && !activeThread && (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-white/90 dark:bg-stone-900/90 backdrop-blur-md p-6 rounded-lg border border-stone-200 dark:border-stone-800 shadow-2xl max-w-md w-full text-center space-y-4 z-20">
           ...
           <button 
             ...
             className="px-6 py-2 bg-emerald-500 text-white rounded-full font-sans text-[10px] uppercase tracking-widest font-black hover:bg-emerald-600 transition-colors disabled:opacity-50"
           >
      ...
      {activeThread && (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-white/95 dark:bg-stone-900/95 backdrop-blur-xl p-8 rounded-lg border border-emerald-500/30 shadow-2xl max-w-lg w-full space-y-6 z-20">
             ...
             {activeThread.path.map((p, i) => {
               ...
               return (
                 <div key={i} className="flex items-center gap-2 shrink-0">
                   <span className={`px-2 py-1 rounded-sm text-[8px] uppercase tracking-widest font-black ${p.type === 'theme' ? 'bg-amber-500/20 text-amber-600 dark:text-amber-500' : 'bg-stone-100 dark:bg-stone-800 text-stone-500 dark:text-stone-400'}`}>
                     {nodeLabel}
                   </span>
