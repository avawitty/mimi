...
export const AuthAction: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-nous-base dark:bg-stone-950 flex flex-col items-center justify-center p-8">
      <div className="max-w-md w-full bg-[#050505] border border-stone-800 p-12 flex flex-col items-center text-center">
        <h1 className="font-serif text-3xl italic mb-6 text-white">Authentication</h1>
        
        {status === 'loading' && (
          <div className="animate-pulse text-stone-400 font-mono text-xs uppercase tracking-widest">
            {message}
          </div>
        )}
        ...
        {status === 'success' && (
          <div className="flex flex-col items-center gap-6">
            ...
            <button 
              ...
              className="font-sans text-xs uppercase tracking-widest font-black text-white border-b border-current pb-1 hover:text-stone-400 transition-colors"
            >
              Return to Vault
            </button>
          </div>
        ...
        {status === 'resetting' && (
          <form onSubmit={handleResetPassword} className="w-full flex flex-col gap-6">
            <div className="text-stone-400 font-mono text-xs uppercase tracking-widest mb-2">
            ...
            <input
              ...
              className="w-full bg-transparent border-b border-stone-800 text-white font-mono text-sm py-2 focus:outline-none focus:border-stone-500 transition-colors"
              required
              minLength={6}
            />
            <button 
              type="submit"
              className="w-full bg-white text-black font-sans text-xs uppercase tracking-widest font-black py-3 hover:bg-stone-200 transition-colors"
            >
              Update Password
            </button>
          </form>
