...
export const UserProfileView: React.FC = () => {
 ...
 return (
 <div className="w-full h-full overflow-y-auto no-scrollbar bg dark:bg-stone-950 text dark:text-stone-100 p-4 md:p-8">
 <AnimatePresence>
 {message && (
 ...
 <span className="text-[10px] uppercase tracking-widest font-mono text-stone-500">Status: Synced</span>
 </div>
 </div>
 </header>
 ...
 className="w-full bg-white dark:bg-stone-900 rounded-none dark: overflow-hidden border border-stone-100 dark:border-stone-800 p-8"
 >
 ...
 <div className="relative w-20 h-20 rounded-none overflow-hidden border border-stone-200 dark:border-stone-700 bg-stone-100 dark:bg-stone-800 flex items-center justify-center shrink-0">
 {avatar ? <img src={avatar} className="w-full h-full object-cover"referrerPolicy="no-referrer"/> : <Camera size={24} className="text-stone-400"/>}
 ...
 <button onClick={() => setExternalLinks([...externalLinks, { title: 'Link', url: '' }])} className="w-full py-2 border border-dashed border-stone-300 dark:border-stone-700 text-[10px] uppercase tracking-widest hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors rounded-none">
 Add Resource
 </button>
 </div>
 </div>
 ...
 {user?.isAnonymous ? (
 <>
 <button onClick={() => handleGoogleLink()} className="w-full flex items-center justify-center gap-2 py-4 border border-stone-800 dark:border-stone-200 text-[10px] uppercase tracking-widest hover:bg-stone-800 hover:text-white dark:hover:bg-stone-200 dark:hover:text-black transition-all rounded-none">
 Sign in with Google
 </button>
 <button onClick={() => {
 ...
 }} className="w-full flex items-center justify-center gap-2 py-4 border border-stone-800 dark:border-stone-200 text-[10px] uppercase tracking-widest hover:bg-stone-800 hover:text-white dark:hover:bg-stone-200 dark:hover:text-black transition-all rounded-none">
 Sign in with Email Link
 </button>
 </>
 ) : (
 <div className="w-full flex items-center justify-center gap-2 py-4 border border-stone-900/30 dark:border-stone-100/30 bg-stone-900/10 dark:bg-stone-100/10 text-[10px] uppercase tracking-widest text-stone-900 dark:text-stone-100 rounded-none">
 ...
 className="w-full bg-white dark:bg-stone-900 rounded-none border border-stone-100 dark:border-stone-800 p-8"
 >
 <div className="flex justify-between items-start mb-6">
 <div>
 ...
 <button onClick={() => window.dispatchEvent(new CustomEvent('mimi:change_view', { detail: 'taste-graph' }))} className="px-8 py-4 bg-stone-800 dark:bg-stone-200 text-white dark:text-black text-[10px] uppercase tracking-[0.2em] hover:bg-stone-700 dark:hover:bg-white transition-colors rounded-none">
 Extract Graph
 </button>
 </div>
 )}
 ...
 className="p-4 border border-stone-200 dark:border-stone-700 text-left hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors rounded-none"
 >
 ...
 <span className="text-[10px] uppercase tracking-widest font-mono px-3 py-1 border border-stone-200 dark:border-stone-700 rounded-none text-stone-500">Agent Config</span>
 </div>
 ...
 <button onClick={() => setArchetype('editorial-serif')} className={`text-[10px] font-mono border px-4 py-2 rounded-none ${archetype === 'editorial-serif' ? 'bg-stone-800 text-white border-stone-800' : 'bg-white dark:bg-stone-800 border-stone-200 dark:border-stone-700'}`}>Editorial</button>
 <button onClick={() => setArchetype('minimalist-sans')} className={`text-[10px] font-mono border px-4 py-2 rounded-none ${archetype === 'minimalist-sans' ? 'bg-stone-800 text-white border-stone-800' : 'bg-white dark:bg-stone-800 border-stone-200 dark:border-stone-700'}`}>Minimalist</button>
 <button onClick={() => setArchetype('brutalist-mono')} className={`text-[10px] font-mono border px-4 py-2 rounded-none ${archetype === 'brutalist-mono' ? 'bg-stone-800 text-white border-stone-800' : 'bg-white dark:bg-stone-800 border-stone-200 dark:border-stone-700'}`}>Brutalist</button>
 </div>
 ...
 />
 <div className="text-[8px] font-mono text-stone-400 text-right mt-1">Budget: {curatorBudget}</div>
 </div>
 <div>
 <div className="flex justify-between items-center mb-2">
 <label className="text-[10px] uppercase tracking-widest font-mono text-stone-500">Sentinel Agent</label>
 <button onClick={() => setSentinelEnabled(!sentinelEnabled)} className={`text-[10px] font-mono px-2 py-1 rounded-none border ${sentinelEnabled ? 'text-stone-900 border-stone-300 bg-stone-100 dark:text-stone-100 dark:border-stone-700 dark:bg-stone-800' : 'text-stone-400 border-stone-200 dark:border-stone-800'}`}>{sentinelEnabled ? 'ENABLED' : 'DISABLED'}</button>
 </div>
 <SemanticSteps 
 steps={[
 { label: 'Low', value: 10 },
 ...
 />
 <div className="text-[8px] font-mono text-stone-400 text-right mt-1">Budget: {sentinelBudget}</div>
 </div>
 </div>
 </div>
 
 <div className="flex flex-col">
 <label className="text-[10px] uppercase tracking-widest font-mono text-stone-500 block mb-3">The Eraser Line</label>
 <textarea 
 ...
 className="font-serif text-lg leading-snug text-stone-600 dark:text-stone-300 border border-stone-200 dark:border-stone-700 rounded-none p-4 bg-stone-50 dark:bg-stone-800/50 resize-none flex-grow focus:outline-none focus:ring-1 focus:ring-stone-400 w-full"
 />
 </div>
 </div>
 </motion.div>
 ...
 <input value={newPersonaName} onChange={e => setNewPersonaName(e.target.value)} placeholder="Mask Name"className="w-full bg-transparent border-b border-stone-300 dark:border-stone-600 p-2 font-serif italic focus:outline-none text-lg"/>
 ...
 <button onClick={handleCreateMask} className="flex-1 py-4 bg-stone-800 text-white text-[10px] uppercase tracking-widest rounded-none hover:bg-stone-700 transition-colors">Mint</button>
 <button onClick={() => setIsAddingPersona(false)} className="flex-1 py-4 border border-stone-200 text-[10px] uppercase tracking-widest rounded-none hover:bg-stone-50 transition-colors">Cancel</button>
 </div>
 </div>
 ...
 <button onClick={() => setIsEditingMask(false)} className="text-stone-400 hover:text-stone-800"><Settings size={16}/></button>
 </div>
 <div>
 <label className="text-[10px] uppercase tracking-widest font-mono text-stone-500 block mb-4">Temperature: {editingMaskTemp}</label>
 <SemanticSteps 
 steps={[
 { label: '0.0', value: 0 },
 { label: '0.5', value: 0.5 },
 ...
 <button onClick={handleUpdateActiveMask} className="w-full py-4 bg-stone-800 text-white text-[10px] uppercase tracking-widest rounded-none hover:bg-stone-700 transition-colors mt-4">Save Parameters</button>
 </div>
 ) : (
 <>
 ...
 <button onClick={prevMask} className="text-stone-400 hover:text-stone-800 dark:hover:text-stone-200 p-2"><ChevronLeft size={24} /></button>
 <span className="font-serif text-4xl italic truncate px-4">{activePersona?.name || 'Personal'}</span>
 <button onClick={nextMask} className="text-stone-400 hover:text-stone-800 dark:hover:text-stone-200 p-2"><ChevronRight size={24} /></button>
 </div>
 </div>
 
 <div className="bg-stone-50 dark:bg-stone-800/50 rounded-none p-4 mb-6 border border-stone-100 dark:border-stone-800">
 ...
 }} className="underline text-stone-800 dark:text-stone-200">Edit</button>
 </div>
 </div>
 
 <button onClick={() => setIsAddingPersona(true)} className="w-full py-4 bg-stone-100 dark:bg-stone-800 text-[10px] uppercase tracking-widest hover:bg-stone-200 dark:hover:bg-stone-700 transition-colors rounded-none font-medium">
 Mint New Mask
 </button>
 </>
 )}
 ...
 <span className="text-[10px] font-mono text-stone-400 bg-stone-50 dark:bg-stone-800 px-3 py-1 rounded-none border border-stone-200 dark:border-stone-700">Minted: {new Date(profile?.createdAt || Date.now()).toLocaleDateString()}</span>
 </div>
 <div className="flex items-center gap-4">
 <div className={`w-12 h-6 rounded-none ${isPatronActive ? 'bg-stone-900 dark:bg-stone-100' : 'bg-stone-200 dark:bg-stone-700'}`}></div>
 ...
 className="flex-1 flex items-center justify-center gap-2 py-4 border border-stone-200 dark:border-stone-700 text-[10px] uppercase tracking-widest hover:bg-stone-50 dark:hover:bg-stone-800 transition-all rounded-none"
 >
 ...
 <button onClick={handleExportData} disabled={isExporting} className="flex items-center justify-center gap-2 px-6 py-4 border border-stone-200 dark:border-stone-700 text-[10px] uppercase tracking-widest hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors rounded-none w-full">
 ...
 <div className="flex flex-col gap-4 w-full md:w-auto">
 <button onClick={handleSave} disabled={isSaving} className="px-8 py-4 bg-stone-800 dark:bg-stone-200 text-white dark:text-black text-[10px] uppercase tracking-widest hover:bg-stone-700 dark:hover:bg-white transition-colors rounded-none font-medium">
