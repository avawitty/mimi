...
export const MobileNavigation: React.FC<MobileNavigationProps> = ({ currentView, setViewMode, profile }) => {
 ...
 let iconColorClass = isActive ? 'text-stone-800' : 'text-stone-400';
 if (tab.id === 'membership' && profile?.plan && profile.plan !== 'free') {
 iconColorClass = profile.plan === 'lab' ? 'text-stone-500' :
 ...
 className="absolute top-0 w-8 h-0.5 bg-stone-800 rounded-none"
 initial={false}
