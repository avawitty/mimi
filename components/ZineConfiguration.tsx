  ...
  return (
    <div className="flex flex-col gap-8 w-full">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 gap-2">
          <button
            ...
            className={`text-left p-3 border rounded-sm transition-colors flex items-center justify-between ${!zineOptions.selectedTreatmentId ? 'border-primary dark:border-white bg-primary/5 dark:bg-white/5' : 'border-stone-200 dark:border-stone-800 hover:border-primary/50 dark:hover:border-white/50'}`}
          >
            <div>
          ...
          {profile?.savedTreatments?.map(t => (
            <button
              ...
              className={`text-left p-3 border rounded-sm transition-colors flex items-center justify-between ${zineOptions.selectedTreatmentId === t.id ? 'border-emerald-500 bg-emerald-500/5' : 'border-stone-200 dark:border-stone-800 hover:border-emerald-500/50'}`}
            >
              <div>
                <p className={`font-serif italic text-sm ${zineOptions.selectedTreatmentId === t.id ? 'text-emerald-500' : 'text-stone-600 dark:text-stone-300'}`}>{t.treatmentName}</p>
                <p className="text-[10px] font-mono text-stone-400 mt-1 line-clamp-1">{t.applicationLogic}</p>
              </div>
              {zineOptions.selectedTreatmentId === t.id && <Check size={14} className="text-emerald-500" />}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className={`flex flex-col gap-4 transition-opacity duration-300 ${zineOptions.selectedTreatmentId ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-4 border border-stone-200 dark:border-stone-800 rounded-sm bg-stone-50 dark:bg-stone-900/30">
