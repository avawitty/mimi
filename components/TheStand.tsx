...
export const TheStand: React.FC<{ onSelectZine: (zine: ZineMetadata) => void }> = ({ onSelectZine }) => {
  ...
  return (
    <div className="flex-1 w-full h-full flex flex-col bg-[#FDFBF7] dark:bg-[#050505] transition-colors duration-1000 relative overflow-hidden">
        ...
        <div className="w-full h-8 bg-nous-text dark:bg-white text-white dark:text-black flex items-center overflow-hidden border-b border-black/5 shrink-0 z-20">
            <div className="flex items-center px-4 h-full bg-emerald-500 text-white shrink-0 font-sans text-[9px] uppercase tracking-widest font-black gap-2">
                ...
                <div className="flex flex-col gap-8">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-end gap-8 pt-12 border-b border-stone-200 dark:border-stone-800 pb-6">
                        ...
                        <div className="flex gap-8">
                            {['FRESH', 'TRENDING', 'DEEP'].map(f => (
                                <button 
                                    ...
                                    className={`font-sans text-[10px] uppercase tracking-[0.2em] font-black pb-2 transition-all ${filter === f ? 'text-emerald-500 border-b-2 border-emerald-500' : 'text-stone-400 hover:text-stone-600 border-b-2 border-transparent'}`}
                                >
                                    {f}
                                </button>
                            ))}
                        ...
                        <div className="relative group w-full md:w-auto">
                            <Search size={14} className="absolute left-0 top-1/2 -translate-y-1/2 text-stone-400 group-focus-within:text-emerald-500 transition-colors" />
                            <input 
                                type="text" 
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                                placeholder="FILTER SIGNAL..." 
                                className="w-full md:w-64 bg-transparent border-b border-stone-200 dark:border-stone-800 py-2 pl-6 font-mono text-xs focus:outline-none focus:border-emerald-500 transition-colors uppercase placeholder:text-stone-300"
                            />
                        </div>
                    </div>
                </div>
