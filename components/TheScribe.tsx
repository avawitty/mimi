...
export const TheScribe: React.FC<TheScribeProps> = ({ onClose, onGenerateZine }) => {
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[50000] bg-stone-100/95 dark:bg-stone-950/95 backdrop-blur-sm flex items-center justify-center p-4 md:p-8 overflow-hidden"
    >
      ...
      <div className="absolute top-8 right-8 flex items-center gap-4 z-20">
        <button 
          onClick={clearCanvas}
          className="p-4 text-stone-400 hover:text-stone-900 dark:hover:text-white transition-colors flex items-center gap-2"
        >
        ...
        <button 
          onClick={onClose}
          className="p-4 text-stone-400 hover:text-stone-900 dark:hover:text-white transition-colors"
        >
      ...
      <motion.div 
        ...
        className="relative w-[95vw] h-[95vw] max-w-[700px] max-h-[700px] rounded-full border border-stone-300 dark:border-stone-800 bg-stone-50/90 dark:bg-stone-900/90 backdrop-blur-md shadow-2xl overflow-hidden flex flex-col items-center justify-center z-10"
      >
        {/* Inner Hairline Ring */}
        <div className="absolute inset-4 rounded-full border border-stone-200 dark:border-stone-800/50 pointer-events-none" />
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center gap-2 transition-all ${
                  isActive ? 'text-stone-900 dark:text-white' : 'text-stone-400 hover:text-stone-600 dark:hover:text-stone-300'
                }`}
              >
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'engine' && (
              <motion.div key="engine" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
                <p className="font-serif italic text-xl text-stone-500">"The mirror awaits your query."</p>
                <div className="relative">
                  <span className="absolute left-0 top-1/2 -translate-y-1/2 font-mono text-stone-400">{'>'}</span>
                  <input 
                    type="text"
                    value={inputValue}
                    onChange={(e) => setInputValue(e.target.value)}
                    placeholder="Search the archive or ask the oracle..."
                    className="w-full bg-transparent border-b border-stone-300 dark:border-stone-700 py-2 pl-6 pr-4 font-mono text-sm text-stone-900 dark:text-white outline-none placeholder:text-stone-300"
                  />
                </div>
              </motion.div>
            )}
                ...
                {scribeTranscript.trim() && onGenerateZine && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-center mt-4">
                    <button
                      ...
                      className="px-4 py-2 border border-stone-200 dark:border-stone-800 rounded-full text-[10px] uppercase tracking-widest hover:bg-stone-100 dark:hover:bg-stone-900 transition-colors flex items-center gap-2"
                    >
                      <Sparkles size={12} />
            ...
            {activeTab === 'synthesis' && (
              <motion.div key="synthesis" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
                <div className="space-y-2">
                  <span className="font-sans text-[9px] uppercase tracking-widest text-stone-400 block">Dual-Agent Protocol</span>
                ...
                {!isGeneratingDebate && !debateAudioUrl && (
                  <>
                    ...
                    <button 
                      ...
                      className="w-full py-3 border border-stone-900 dark:border-white text-stone-900 dark:text-white font-sans text-[9px] uppercase tracking-widest font-black hover:bg-stone-900 hover:text-white dark:hover:bg-white dark:hover:text-black transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-center gap-4">
                      <button 
                        ...
                        className="w-12 h-12 rounded-full border border-stone-900 dark:border-white flex items-center justify-center text-stone-900 dark:text-white hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors"
                      >
                    ...
                    <div className="h-48 overflow-y-auto text-left space-y-4 pr-2 scrollbar-thin scrollbar-thumb-stone-300 dark:scrollbar-thumb-stone-700">
                      {debateTranscript.map((turn, idx) => (
                        <div key={idx} className={`space-y-1 ${turn.speaker === 'Mimi' ? 'pl-4 border-l-2 border-stone-300 dark:border-stone-700' : 'pr-4 border-r-2 border-stone-300 dark:border-stone-700 text-right'}`}>
                          <span className="font-sans text-[8px] uppercase tracking-widest text-stone-400 font-black">
                            {turn.speaker}
                          </span>
                      ...
                      {synthesisText && (
                        <div className="pt-4 mt-4 border-t border-stone-200 dark:border-stone-800">
                          <span className="font-sans text-[8px] uppercase tracking-widest text-stone-400 font-black block mb-2">
                            Synthesis
                          </span>
                    ...
                    <button 
                      ...
                      className="text-[10px] font-mono text-stone-400 hover:text-stone-900 dark:hover:text-white uppercase tracking-widest underline underline-offset-4"
                    >
                      Reset Protocol
                    </button>
                  </div>
        ...
        {/* Bottom Equator Line */}
        <div className="absolute bottom-24 w-1/2 h-px bg-stone-200 dark:bg-stone-800" />
        <span className="absolute bottom-16 font-mono text-[8px] text-stone-400 uppercase tracking-widest">
