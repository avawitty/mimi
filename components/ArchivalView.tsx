...
import { Loader2, X, Check, Filter, ArrowLeft, Plus, FolderPlus, Archive, Trash2, Radar, Briefcase, ScrollText, Pencil, Download, Mic, Film, Search, Play, Pause, Clapperboard, Video as VideoIcon, Save, StickyNote, PenTool, Link as LinkIcon, Wallet, Paperclip, Waves, Info, Binary, ExternalLink, Tag, MapPin, Hash, User, ImageIcon, DollarSign, CornerDownRight, Square, Layers, Target, Activity, Sparkles, FolderOpen } from 'lucide-react';
...
const SkeletonCard = () => (
  <div className="flex flex-col gap-3 animate-pulse">
    ...
    <div className="space-y-2 pt-1">
      ...
      <div className="h-2 w-1/4 bg-stone-100 dark:bg-stone-900 rounded-sm" />
    </div>
  </div>
);
...
// --- MOODBOARD ITEM (The "Polaroid" Style) ---
const MoodboardItem = ({ item, isSelected, onClick, onDragStart }) => {
    // Generate deterministic rotation based on ID
    ...
    const isText = item.type === 'text' || item.type === 'voicenote';
    ...
    return (
        <motion.div
            ...
            className={`
                relative cursor-pointer transition-all duration-500 break-inside-avoid mb-8
                ${isSelected ? 'ring-2 ring-emerald-500 ring-offset-4 ring-offset-transparent' : ''}
            `}
        >
            {isImage ? (
                <div className="bg-white p-3 pb-8 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1),0_2px_4px_-1px_rgba(0,0,0,0.06),0_0_2px_rgba(0,0,0,0.1)] dark:bg-stone-900 dark:border dark:border-stone-800">
                    <div className="aspect-[4/5] w-full bg-stone-100 dark:bg-black overflow-hidden relative mb-3">
                         <img src={item.content.imageUrl} className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-700" loading="lazy" />
                         {/* Tape Effect */}
                         <div className="absolute -top-3 left-1/2 -translate-x-1/2 w-16 h-6 bg-white/30 backdrop-blur-sm rotate-[-2deg] border border-white/20 shadow-sm" />
                    </div>
                    <div className="px-2">
                        <p className="font-serif italic text-lg text-stone-800 dark:text-stone-200 leading-tight mb-2 line-clamp-2">
                            {item.content.prompt || "Untitled"}
                        </p>
                        <div className="flex justify-between items-center border-t border-stone-100 dark:border-stone-800 pt-2">
                            <span className="font-mono text-[8px] text-stone-400">{new Date(item.savedAt).toLocaleDateString()}</span>
                            <span className="font-mono text-[8px] text-stone-400 uppercase tracking-widest">IMG</span>
                        </div>
                    </div>
                </div>
            ) : (
                <div className="bg-[#FEF9C3] dark:bg-stone-800 p-6 shadow-md hover:shadow-lg relative min-h-[180px] flex flex-col justify-between transform transition-transform">
                    {/* Tape Effect */}
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 w-20 h-8 bg-white/40 backdrop-blur-sm rotate-1 border border-white/20 shadow-sm" />
                    
                    <div className="space-y-4 pt-2">
                        {item.type === 'voicenote' && <Mic size={16} className="text-stone-400 mb-2" />}
                        <p className="font-serif italic text-xl text-stone-800 dark:text-stone-200 leading-snug">
                            "{item.content.prompt || item.notes || item.content.transcript || "Empty note."}"
                        </p>
                    </div>
                    
                    <div className="flex justify-end mt-4 pt-4 border-t border-black/5 dark:border-white/5">
                        <span className="font-mono text-[9px] text-stone-500 uppercase tracking-widest">{item.type}</span>
                    </div>
                </div>
            )}
        </motion.div>
...
export const ArchivalView = ({ onSelectZine }) => {
    ...
    const handleSaveAudit = async () => {
        ...
        try {
            await addToPocket(user?.uid || 'ghost', 'analysis_report', {
                ...
                timestamp: Date.now(),
                // NEW: Ensure color story and touchpoints are explicitly saved in the content payload
                colorStory: auditReport.colorStory,
                keyTouchpoints: auditReport.keyTouchpoints
            });
    ...
    const handleProposal = () => {
        // Direct folder proposal trigger
        if (activeBoard) {
            setTempProposalData({
                id: activeBoard.id, // Pass ID to link source
                name: activeBoard.content.name, 
                items: items.filter(i => activeBoard.content.itemIds?.includes(i.id)), 
                    ...
                    <div className="space-y-4">
                        ...
                        {activeBoard && (
                            <div className="flex gap-4">
                                <button onClick={() => setActiveBoard(null)} className="flex items-center gap-2 font-sans text-[8px] uppercase font-black text-stone-400 hover:text-emerald-500"><ArrowLeft size={10} /> Back to Repository</button>
                                <button onClick={handleProposal} className="flex items-center gap-2 font-sans text-[8px] uppercase font-black text-emerald-500 hover:text-emerald-400 transition-colors bg-emerald-500/10 px-3 py-1 rounded-full border border-emerald-500/20">
                                    <Sparkles size={10} /> Initialize Proposal
                                </button>
                            </div>
                        )}
                    </div>
