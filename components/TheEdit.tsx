...
import { getPersonalizedEdit } from '../services/commerceService';
...
const ProductCard = ({ product, onInteraction }: { product: Product; onInteraction: (id: string, signal: any) => void }) => {
  ...
  return (
    <div ref={ref} className="group relative border border-[#D4D1C9] bg-white p-4 transition-all hover:border-black">
      <div className="aspect-[4/5] bg-stone-100 mb-4 flex items-center justify-center overflow-hidden">
        {product.image ? <img src={product.image} alt={product.name} className="w-full h-full object-cover" /> : <span className="font-serif italic text-stone-400">{product.name}</span>}
      </div>
      <div className="flex justify-between items-end">
        ...
        <button className="font-mono text-[9px] uppercase underline">Acquire</button>
      </div>
    </div>
  );
};
...
export const TheEdit: React.FC = () => {
    ...
    if (user) {
      const fetchData = async () => {
        try {
          ...
          getPersonalizedEdit(user.uid, tasteVector, codex).then(edit => {
  ...
  useEffect(() => {
    if (personalizedEdit?.sequence) {
      const fetchProducts = async () => {
        try {
          const productPromises = personalizedEdit.sequence.map(item => getDoc(doc(db, 'products', item.productId)));
  ...
  return (
    <div className="min-h-screen bg-[#F9F7F2] text-[#1A1A1A] font-sans flex">
      <nav className="w-20 border-r border-[#D4D1C9] flex flex-col items-center py-12 fixed h-full z-50 bg-[#F9F7F2]">
      ...
      <main className="ml-20 flex-1 p-12">
        <header className="mb-16 border-b border-[#D4D1C9] pb-8 flex justify-between items-end">
          <div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex bg-[#D4D1C9] p-1 rounded-full">
              <button onClick={() => setViewMode('editorial')} className={`px-4 py-1 rounded-full text-[10px] uppercase tracking-widest flex items-center gap-2 ${viewMode === 'editorial' ? 'bg-white shadow-sm' : ''}`}><BookOpen size={12} /> Editorial</button>
              <button onClick={() => setViewMode('market')} className={`px-4 py-1 rounded-full text-[10px] uppercase tracking-widest flex items-center gap-2 ${viewMode === 'market' ? 'bg-white shadow-sm' : ''}`}><LayoutGrid size={12} /> Market</button>
            </div>
        ...
        {personalizedEdit ? (
          <section className="grid grid-cols-12 gap-8">
            <article className="col-span-12 lg:col-span-4 bg-[#F9F7F2] p-8 border border-[#D4D1C9]">
