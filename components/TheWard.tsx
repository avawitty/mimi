...
const DriftMonitor: React.FC<{ zines: ZineMetadata[] }> = ({ zines }) => {
  ...
  return (
    <motion.div 
      ...
      className="mb-8 p-4 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/50 rounded-sm flex items-start gap-4"
    >
...
export const TheWard: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  useEffect(() => {
    if (user?.uid && !user.uid.startsWith('local_ghost_') && user.uid !== 'ghost') {
      getTasteGraph(user.uid).then(graph => setNodes(graph.nodes));
