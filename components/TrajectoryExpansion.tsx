...
export const TrajectoryExpansion: React.FC<{ article: any, onClose: () => void }> = ({ article, onClose }) => {
  return (
    <motion.div 
      ...
      exit={{ opacity: 0, y: 20 }}
      className="fixed inset-0 z-[6000] bg-[#F4F1EA] dark:bg-[#0A0A0A] overflow-y-auto flex flex-col md:flex-row"
    >
      <button onClick={onClose} className="fixed top-6 right-6 z-50 p-4 bg-black text-white dark:bg-white dark:text-black rounded-full hover:scale-105 transition-transform shadow-xl">
        <X size={20} />
      </button>
      ...
      <div className="w-full md:w-1/2 h-[50vh] md:h-screen sticky top-0 border-b md:border-b-0 md:border-r border-stone-300 dark:border-stone-800 p-4 md:p-8">
        <div className="w-full h-full relative overflow-hidden bg-stone-200 dark:bg-stone-900 rounded-xl shadow-2xl">
           {article.image ? (
      ...
      <div className="w-full md:w-1/2 min-h-screen p-8 md:p-16 lg:p-24 flex flex-col bg-[#F9F8F6] dark:bg-[#050505]">
        <div className="mb-16">
          <div className="flex items-center gap-4 mb-8">
            <span className="font-mono text-[10px] uppercase tracking-widest px-3 py-1 border border-stone-300 dark:border-stone-700 rounded-full text-stone-600 dark:text-stone-400">
              {article.tag}
            </span>
            <span className="font-mono text-[10px] uppercase tracking-widest text-emerald-600 dark:text-emerald-500 flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-12">
          <div className="xl:col-span-2 space-y-8">
            ...
            {article.content.outro && (
              <div className="p-6 bg-stone-100 dark:bg-stone-900 rounded-lg border border-stone-200 dark:border-stone-800">
