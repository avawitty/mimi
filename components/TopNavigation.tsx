  ...
  return (
    <nav className="flex items-center gap-6">
        ...
        <React.Fragment key={section.section}>
          <div className="relative group">
            ...
            <div className="absolute top-full left-0 pt-2 w-48 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none group-hover:pointer-events-auto z-[5000]">
              <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 p-4 shadow-xl flex flex-col gap-2">
                {section.items.map((item) => (
                  <button
                    key={item.mode}
                    onClick={() => setViewMode(item.mode)}
                    className={`text-[10px] uppercase tracking-[0.1em] text-left hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors ${currentViewMode === item.mode ? 'text-emerald-600 dark:text-emerald-400 font-bold' : 'text-stone-600 dark:text-stone-400'}`}
                  >
                    {item.label}
                  </button>
                ))}
      ...
      <button 
        ...
        className="font-sans text-[10px] uppercase tracking-[0.2em] font-medium text-emerald-600 dark:hover:text-emerald-400 transition-colors"
      >
        Command
      </button>
    </nav>
