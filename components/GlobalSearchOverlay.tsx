    ...
    >
      <div className="w-full max-w-4xl flex flex-col h-full">
        <header className="flex justify-between items-center mb-12 shrink-0">
          ...
          <button onClick={onClose} className="p-3 bg-stone-100 dark:bg-stone-800 rounded-full hover:bg-red-500 hover:text-white transition-all"><X size={20}/></button>
        </header>
        ...
        <div className="relative mb-16 shrink-0">
          <input 
            ...
            className="w-full bg-transparent border-b-2 border-stone-100 dark:border-stone-800 py-8 font-serif italic text-3xl md:text-6xl text-nous-text dark:text-white focus:outline-none focus:border-emerald-500 transition-all placeholder:text-stone-200"
          />
          {loading && <div className="absolute right-4 top-1/2 -translate-y-1/2"><Loader2 className="animate-spin text-emerald-500" /></div>}
        </div>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar space-y-16 pb-32">
          {query.trim() ? (
                ...
                <section className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {filtered.pocket.map(p => (
                      <div 
                        key={p.id} 
                        className="p-5 bg-stone-50 dark:bg-stone-900/50 border border-stone-100 dark:border-stone-800 rounded-sm flex gap-4 items-center group cursor-help"
                      >
                         <div className="w-12 h-12 bg-black overflow-hidden rounded-sm flex-shrink-0">
                            {p.type === 'image' && <img src={p.content.imageUrl} className="w-full h-full object-cover grayscale group-hover:grayscale-0" />}
                            {p.type !== 'image' && <div className="w-full h-full flex items-center justify-center text-emerald-500"><Radio size={16}/></div>}
                         </div>
            ...
            <div className="h-full flex flex-col items-center justify-center opacity-20 space-y-8 py-20">
               <div className="p-10 border border-stone-200 dark:border-stone-800 rounded-full">
