...
export const SanctuaryView: React.FC = () => {
  ...
  return (
    <div className="flex-1 w-full h-full overflow-y-auto no-scrollbar pb-64 px-6 md:px-16 pt-12 md:pt-20 bg-[#050505] text-white transition-all duration-1000 relative selection:bg-white selection:text-black">
            ...
            <motion.div key="begin" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-16">
               <div className="grid md:grid-cols-2 gap-16">
                  <div className="space-y-8">
                     <textarea value={sanctuaryInput} onChange={(e) => setSanctuaryInput(e.target.value)} className="w-full bg-stone-900/50 border border-white/10 p-6 font-serif text-xl italic text-white focus:outline-none focus:border-emerald-500/50 transition-all resize-none h-48 rounded-sm" placeholder="The frequency feels distorted..." />
                     <button onClick={handleSanctuaryCalibration} disabled={isLoading || !sanctuaryInput.trim()} className="px-10 py-4 bg-white text-black rounded-sm font-sans text-[9px] uppercase tracking-[0.4em] font-black hover:bg-emerald-400 transition-all flex items-center gap-4 disabled:opacity-50">
                  ...
                  <div className="border-l border-white/5 pl-16 flex flex-col justify-center">
                     {sanctuaryReport ? (
                       <div className="space-y-12">
                          <div className="space-y-2">
                             <span className="font-sans text-[8px] uppercase tracking-widest text-emerald-500 font-black">Validation</span>
