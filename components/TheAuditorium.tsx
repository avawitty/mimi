...
const EPISODES = [
  ...
  },
  {
    ...
    episode: "Episode 3",
    desc: "The ROI of being seen. Reconciling with the high-fidelity shadow.",
  ...
  return (
    <div className="flex-1 w-full h-full overflow-y-auto no-scrollbar pb-64 px-6 md:px-16 pt-12 md:pt-20">
      <div className="max-w-7xl mx-auto space-y-16">
        ...
        <section className="relative aspect-video w-full bg-black rounded-[2rem] overflow-hidden shadow-[0_50px_100px_rgba(0,0,0,0.4)] border border-stone-800 group">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center space-y-8 z-20">
                <motion.div 
                  ...
                  className="w-24 h-24 rounded-full border border-emerald-500/30 flex items-center justify-center backdrop-blur-xl"
                >
                  <Play size={32} className="text-white fill-current ml-2" />
                </motion.div>
                
                <div className="space-y-4">
                  <span className="font-sans text-[10px] uppercase tracking-[1em] text-emerald-500 font-black">Now Playing // {activeEp.episode}</span>
                ...
                <a 
                  href={activeEp.link} 
                  target="_blank" 
                  className="px-10 py-4 bg-white text-black font-sans text-[10px] uppercase tracking-[0.5em] font-black rounded-full shadow-2xl active:scale-95 transition-all flex items-center gap-4 hover:bg-emerald-500 hover:text-white"
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
           {EPISODES.map((ep) => (
             <button 
               ...
               className={`text-left p-8 rounded-2xl border transition-all flex flex-col gap-6 group relative overflow-hidden ${activeEp.id === ep.id ? 'bg-white dark:bg-stone-900 border-nous-text dark:border-white shadow-2xl scale-105' : 'bg-stone-50 dark:bg-stone-950 border-stone-100 dark:border-stone-800 opacity-60 hover:opacity-100'}`}
             >
                <div className="flex justify-between items-start">
                   <span className="font-sans text-[8px] uppercase tracking-widest font-black text-stone-400">{ep.episode}</span>
                   {activeEp.id === ep.id && <Activity size={12} className="text-emerald-500 animate-pulse" />}
                </div>
        ...
        {/* THE LINK MANDATE */}
        <section className="bg-amber-50/30 dark:bg-amber-950/10 border border-amber-100 dark:border-amber-900/30 p-12 rounded-[3rem] flex flex-col md:flex-row items-center gap-12">
            <div className="p-8 bg-white dark:bg-stone-900 rounded-full shadow-2xl border border-amber-200">
            ...
            <div className="w-full md:w-auto">
               <button 
                 ...
                 className="w-full px-10 py-6 bg-nous-text dark:bg-stone-100 text-white dark:text-stone-950 font-sans text-[10px] uppercase tracking-[0.6em] font-black rounded-full shadow-2xl active:scale-95 transition-all border border-white/20"
               >
