...
export const ZineCoverCard: React.FC<ZineCoverCardProps> = ({ zine, onClick }) => {
  ...
  return (
    <>
      ...
      <div 
        onClick={onClick}
        className="group relative w-full aspect-[3/4] rounded-t-sm rounded-b-xl overflow-hidden shadow-[0_20px_40px_-15px_rgba(0,0,0,0.3)] flex flex-col cursor-pointer hover:scale-[1.02] transition-transform duration-500"
        style={{ 
            ...
            <div className="absolute inset-0 bg-black/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-4 md:p-6 flex flex-col justify-between text-white">
                ...
                <div className="flex flex-wrap gap-1.5 mt-4">
                    {(zine.tags && zine.tags.length > 0 ? zine.tags : (zine.theme ? [zine.theme] : [])).slice(0, 4).map((tag, idx) => (
                        <span key={idx} className="px-2 py-1 bg-white/10 rounded-sm text-[8px] uppercase tracking-widest font-sans font-bold text-white/80">
                            {tag}
                        </span>
                    ))}
                </div>
