...
export const MimiGawdTerminal: React.FC<MimiGawdTerminalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-[9999] bg-[#070709] overflow-y-auto overflow-x-hidden"
      ref={containerRef}
    >
      {/* Grain Overlay */}
      <div 
      ...
      <button 
        onClick={onClose}
        className="fixed top-8 right-8 z-[100] text-[#ff3333] font-mono text-sm tracking-widest hover:text-white transition-colors opacity-50 hover:opacity-100"
      >
        [ DISCONNECT ]
      </button>
