...
export const CollabModal: React.FC<CollabModalProps> = ({
 ...
 <div className="bg p-8 rounded-none max-w-lg w-full space-y-6 border border-stone-800">
 ...
 className="flex-1 bg-stone-900/50 border border-stone-800 p-3 font-mono text-sm focus:outline-none focus:border-stone-800 dark:focus:border-stone-300 rounded-none text-stone-300 placeholder:text-stone-700"
 />
 <button onClick={handleSearchCollaborators} className="px-4 py-2 bg-stone-500 text-black rounded-none font-mono text-[9px] uppercase font-bold tracking-widest hover:bg-stone-600 transition-colors">Search</button>
 </div>
 {isSearchingCollab && <Loader2 size={20} className="animate-spin text-stone-500 mx-auto"/>}
 <div className="space-y-2 max-h-60 overflow-y-auto">
 {collabSearchResults.map(profile => (
 <div key={profile.uid} className="flex justify-between items-center p-2 border border-stone-800 rounded-none">
 <span className="font-mono text-sm text-stone-300">{profile.handle}</span>
 <button onClick={() => handleAddCollaborator(profile.uid)} className="text-stone-500 hover:text-stone-400"><Plus size={16} /></button>
 </div>
 ))}
 </div>
