...
const TONE_MAP: Record<ToneTag, { bg: string, text: string, accent: string }> = {
 ...
 'Editorial Stillness': { bg: 'bg', text: 'text', accent: 'border-stone-200' },
 ...
 'chic': { bg: 'bg', text: 'text-stone-900', accent: 'border-stone-300' },
...
export const ArchiveCloudNebula: React.FC<{ onSelectZine: (zine: ZineMetadata) => void, onGenerateThreadZine?: (thread: any) => void }> = ({ onSelectZine, onGenerateThreadZine }) => {
 ...
 className="w-full bg-stone-50/50 dark:bg-stone-900/50 border-b-2 border-stone-100 dark:border-stone-800 py-10 pl-40 pr-8 font-serif italic text-2xl md:text-3xl focus:outline-none focus:border-stone-800 dark:focus:border-stone-300 transition-all placeholder:text-stone-200 placeholder:opacity-50 text-nous-text dark:text-white"
 />
 </div>
 ...
 className="bg-white dark:bg border border-stone-200 dark:border-stone-800 p-6 flex flex-col items-center text-center cursor-pointer hover:border-indigo-500 transition-colors group"
 >
 <div className="w-16 h-16 rounded-none overflow-hidden mb-4 bg-stone-100 dark:bg-stone-900 border border-stone-200 dark:border-stone-800">
...
const ZineShelfItem: React.FC<{ zine: ZineMetadata, onSelect: () => void, isCloud?: boolean, isStarred?: boolean, onToggleStar: () => void, mode?: string }> = ({ zine, onSelect, isCloud, isStarred, onToggleStar, mode = 'dossier' }) => {
 ...
 viewport={{ once: true }}
 className="col-span-1 md:col-span-6 lg:col-span-4 bg dark:bg-stone-900 flex flex-col relative overflow-hidden"
 >
 ...
 {hasNote && (
 <div className={`absolute bg/90 dark:bg-stone-800/90 border border dark:border-stone-700 backdrop-blur-sm p-3 w-36 z-20 ${notePosition} ${noteRotation}`}>
 <div className="absolute -top-1.5 left-1/2 -translate-x-1/2 -rotate-2 w-10 h-3 bg-white/40 dark:bg-white/10 border border-black/10 dark:border-white/10"></div>
...
const InitSequenceCard: React.FC<{ mode: string }> = ({ mode }) => {
 ...
 whileInView={{ opacity: 1, y: 0 }}
 viewport={{ once: true }}
 className="col-span-1 md:col-span-6 lg:col-span-4 bg-stone-800 text-stone-200 p-8 flex flex-col justify-between rounded-none hover: transition-"
 >
 <div>
