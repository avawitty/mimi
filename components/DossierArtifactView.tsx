...
export const DossierArtifactView: React.FC<{ artifact: DossierArtifact; onClose: () => void }> = ({ artifact, onClose }) => {
 ...
 return (
 <div className="fixed inset-0 z-[5000] bg text-stone-200 overflow-y-auto no-scrollbar font-mono selection:bg-stone-500/30 selection:text-stone-200">
 {/* Grid Background */}
 <div className="fixed inset-0 bg bg-[size:40px_40px] opacity-20 pointer-events-none"/>

 <div className="h-20 border-b border-stone-800 px-6 md:px-12 flex justify-between items-center sticky top-0 bg/90 backdrop-blur-xl z-[100] print:hidden">
 <button onClick={onClose} className="flex items-center gap-4 group">
 <div className="p-2 border border-stone-800 rounded-none group-hover:border-stone-800 dark:hover:border-stone-300 group-hover:text-stone-800 dark:hover:text-stone-300 transition-all"><X size={16} /></div>
 <span className="font-mono text-[9px] uppercase tracking-[0.2em] font-bold text-stone-500 group-hover:text-stone-800 dark:hover:text-stone-300 transition-colors">Close Artifact</span>
 </button>
 <div className="flex items-center gap-4">
 <button onClick={handleShare} className="flex items-center gap-3 px-6 py-2 bg-stone-900 border border-stone-800 rounded-none font-mono text-[9px] uppercase tracking-widest font-bold text-stone-500 hover:text-stone-500 hover:border-stone-500/50 transition-all">
 {copied ? <Check size={12} className="text-stone-500"/> : <Share2 size={12} />}
 {copied ? 'Link Copied' : 'Share Shard'}
 </button>
 <button onClick={handleExportPdf} className="p-3 bg-stone-900 border border-stone-800 text-stone-400 hover:text-stone-500 hover:border-stone-500/50 rounded-none transition-all">
 <Download size={18} />
 </button>
 </div>
 </div>
 ...
 <Target size={48} className="mx-auto text-stone-500 opacity-50"/>
 ...
 <span className="font-mono text-sm text-stone-500 uppercase tracking-widest border border-stone-500/20 bg-stone-500/5 px-3 py-1 rounded-none">{artifact.type || 'Unknown'}</span>
 </div>
 ...
 <div className="bg-stone-900 p-1 border border-stone-800 rounded-none relative overflow-hidden group-hover:border-stone-500/30 transition-all duration-500">
 ...
 <Pin size={12} className={`text-stone-500 ${el.style?.hasPin ?"opacity-100":"opacity-0"}`} />
 </div>
 ...
 <div className="flex-1 bg-stone-900/50 p-8 flex flex-col justify-start text-left border border-stone-800 hover:border-stone-500/30 transition-colors rounded-none relative overflow-hidden">
 <div className="absolute top-0 left-0 w-1 h-full bg-stone-800 group-hover:bg-stone-500 transition-colors"/>
 ...
 <div>
 <h4 className="font-mono text-[10px] uppercase tracking-widest text-stone-500 mb-2">Opening Line</h4>
 <p className="italic text-stone-400">"{data.openingLine}"</p>
 </div>
 <div>
 <h4 className="font-mono text-[10px] uppercase tracking-widest text-stone-500 mb-2">Aesthetic Audit</h4>
 ...
 <div>
 <h4 className="font-mono text-[10px] uppercase tracking-widest text-stone-500 mb-2">Strategy Shift</h4>
 <ul className="list-disc pl-4 space-y-1 text-stone-400">
 {data.strategyShift.map((s: string, i: number) => <li key={i}>{s}</li>)}
 </ul>
 </div>
 <div>
 <h4 className="font-mono text-[10px] uppercase tracking-widest text-stone-500 mb-2">Identity Reframe</h4>
 <p className="italic text-stone-400">"{data.identityReframe}"</p>
 </div>
 </div>
 );
 } catch (e) {
 return <pre className="font-mono text-[10px] text-stone-400 overflow-x-auto p-4 bg-black/50 rounded-none border border-stone-800">{el.content}</pre>;
 }
 })()
 ) : (
 `"${el.content}"`
 ...
 <div className="p-8 bg-stone-900/30 border border-stone-800 rounded-none space-y-8">
 <div className="space-y-2">
 <span className="font-mono text-[8px] uppercase tracking-widest font-bold text-stone-500">Frequency Audit</span>
 ...
 <div key={i} className="w-8 h-8 rounded-none border border-stone-700"style={{ backgroundColor: c.hex }} title={c.name} />
 ))}
 </div>
 </div>
 </div>
 ...
 <button onClick={onClose} className="px-12 py-4 border border-stone-800 hover:border-stone-500 text-stone-500 hover:text-stone-500 font-mono text-[9px] uppercase tracking-[0.4em] font-bold rounded-none transition-all">Return to Grid</button>
 </footer>
 </div>
 </div>
 );
