...
export const MoodboardComposer: React.FC<MoodboardComposerProps> = ({ selectedItems, report, onCancel, onFinalize }) => {
 ...
 >
 <header className="h-20 border-b border-stone-800 px-8 flex justify-between items-center bg z-[100]">
 <button onClick={onCancel} className="flex items-center gap-4 group">
 <div className="p-2 border border-stone-800 group-hover:bg-stone-900 group-hover:text-stone-300 transition-all text-stone-500"><X size={16} /></div>
 <span className="font-mono text-[9px] uppercase tracking-widest font-bold text-stone-500 group-hover:text-stone-300">Cancel Manifest</span>
 </button>
 
 <div className="flex items-center gap-6">
 <div className="flex flex-col items-end">
 <span className="font-mono text-[9px] uppercase tracking-widest text-stone-500 font-bold">Composition Cycle</span>
 <span className="font-serif italic text-sm text-stone-300">{elements.length} Constituent Fragments</span>
 </div>
 <button onClick={() => onFinalize(elements, { backgroundColor: '#050505', aspectRatio:"16:9"})} className="px-6 py-3 border border-stone-800 text-stone-300 font-mono text-[9px] uppercase tracking-widest font-bold hover:bg-stone-900 hover:text-white transition-all flex items-center gap-3">
 ...
 <aside className="w-64 border-r border-stone-800 p-8 overflow-y-auto bg">
 ...
 <div className="text-center space-y-4 mb-20 border-b border-stone-800 pb-8">
 ...
 <button onClick={() => togglePin(el.id)} className={`p-2 border transition-all ${el.style.hasPin ? 'bg-stone-800 text-stone-200 border-stone-500' : 'bg-transparent text-stone-500 border-stone-800 hover:border-stone-600'}`}>
 <Pin size={12} fill={el.style.hasPin ?"currentColor":"none"} />
 </button>
 <button onClick={() => removeElement(el.id)} className="p-2 bg-transparent text-stone-500 border border-stone-800 hover:text-red-500 hover:border-red-900/50 transition-all">
 <Trash2 size={12} />
 </button>
 </div>
 ...
 <div className={`p-3 bg-transparent border border-stone-800 transition-all duration-700 ${
 ...
 <div className="pt-4 pb-1 px-1 opacity-20 group-hover:opacity-100 transition-opacity flex justify-between items-center border-t border-stone-800 mt-3">
 <span className="font-mono text-[9px] uppercase tracking-widest text-stone-500">shrd_0{idx+1}</span>
 <span className="font-mono text-[9px] uppercase tracking-widest font-bold text-stone-500">IMAGE</span>
 </div>
 </div>
 {el.notes && (
 <div className="px-2 space-y-2 opacity-60 group-hover:opacity-100 transition-opacity border-l border-stone-800 pl-4">
 <div className="flex items-center gap-2 text-stone-500">
 <Quote size={10} />
 <span className="font-mono text-[9px] uppercase tracking-widest font-bold">Linked Remark</span>
 </div>
 <p className="font-serif italic text-sm text-stone-400 line-clamp-3">"{el.notes}"</p>
 </div>
 )}
 </div>
 ) : (
 <div className={`p-8 bg-transparent border border-stone-800 h-full flex flex-col justify-start text-left transition-all duration-700`}>
 ...
 <span className="font-mono text-[9px] uppercase tracking-widest text-stone-600">REF_0{idx+1}</span>
 </div>
 <div className="flex-1 overflow-y-auto max-h-[400px] no-scrollbar">
 <p className={`${getTypographyClass()} text-stone-300 whitespace-pre-wrap ${el.type === 'analysis_pin' ? 'text-xl md:text-2xl leading-snug' : 'text-lg md:text-xl leading-relaxed'}`}>
"{el.content}"
 </p>
 </div>
 {el.notes && (
 <div className="mt-8 pt-4 border-t border-stone-800 opacity-60">
 <span className="font-mono text-[9px] uppercase tracking-widest font-bold text-stone-500 block mb-2">Field Note</span>
 <p className="font-serif italic text-xs text-stone-400 line-clamp-2 leading-tight">"{el.notes}"</p>
 </div>
 )}
 </div>
 )}
