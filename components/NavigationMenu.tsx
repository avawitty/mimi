        ...
        >
          ...
          <AnimatePresence>
            {hoveredItem === category && (
              ...
              >
                <div className="bg-stone-900 border border-stone-800 p-4 shadow-xl flex flex-col gap-2">
                  {items.map((item) => (
                    <button
                      key={item.viewMode}
                      onClick={() => setViewMode(item.viewMode)}
                      className={`text-[10px] uppercase tracking-[0.1em] text-left hover:text-emerald-400 transition-colors ${currentViewMode === item.viewMode ? 'text-white font-bold' : 'text-stone-400'}`}
                    >
                      {item.label}
                    </button>
                  ))}
