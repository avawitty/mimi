...
export const MobileStudio: React.FC<MobileStudioProps> = ({ onPublish, onClose, onOpenProfile }) => {
  ...
  return (
    <div className="h-full w-full bg-[#FDFBF7] flex flex-col font-serif overflow-hidden">
      ...
      <div className="pb-28 px-6 flex flex-col items-center w-full">
        ...
        <div className="w-full max-w-[320px] overflow-x-auto no-scrollbar mb-8">
          <div className="flex items-center justify-start gap-6 px-6 py-3 bg-white/80 backdrop-blur-xl rounded-full shadow-[0_8px_32px_rgba(0,0,0,0.06)] border border-white/60 text-stone-800 min-w-max">
          ...
          <div className="flex items-center justify-center gap-3 mb-6 overflow-x-auto w-full no-scrollbar px-4">
            {profile.savedTreatments.map(t => (
              <button
                ...
                className={`whitespace-nowrap px-3 py-1.5 text-[10px] uppercase tracking-widest rounded-sm border transition-colors ${activeTreatmentId === t.id ? 'border-emerald-500 text-emerald-500 bg-emerald-500/10' : 'border-stone-300 dark:border-stone-700 text-stone-500 hover:border-stone-400 dark:hover:border-stone-500'}`}
              >
                [{t.treatmentName}]
              </button>
            ))}
        ...
        <div className="flex items-center justify-center gap-5 mb-12 overflow-x-auto w-full no-scrollbar text-[15px] italic text-stone-300 px-4">
          {TAGS.map(tag => (
            <button 
              ...
              className={`whitespace-nowrap pb-1 border-b-[1.5px] transition-colors ${activeTag === tag ? 'text-stone-600 border-emerald-500' : 'border-transparent hover:text-stone-400'}`}
            >
              {tag}
            </button>
          ))}
        ...
        >
          ...
          <div className="flex items-center gap-2 text-[9px] font-sans uppercase tracking-[0.2em] text-stone-400">
            <div className={`w-1.5 h-1.5 rounded-full ${input.trim() ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.6)]' : 'bg-stone-300'}`}></div>
            System Nominal
          </div>
        </button>
      </div>
