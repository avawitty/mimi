...
export const ThreadGraph: React.FC<ThreadGraphProps> = ({ metadata, accentColor }) => {
  ...
  return (
    <div className="relative w-full h-[400px] md:h-[500px] bg-stone-900 rounded-xl overflow-hidden border border-stone-800 p-8 shadow-2xl">
      {/* Background Grid */}
        ...
        return (
          ...
          >
            <div className="relative">
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-black border border-stone-700 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300 z-10 relative" style={{ borderColor: node.type === 'motif' ? accentColor : undefined }}>
            ...
            <div className="mt-3 text-center w-32">
              <span className="font-mono text-[8px] md:text-[9px] uppercase tracking-widest text-stone-400 group-hover:text-white transition-colors bg-black/50 px-2 py-1 rounded backdrop-blur-sm">
                {node.label}
              </span>
            </div>
          </motion.div>
