...
export function ThimbleIndex() {
  ...
  return (
    <div className="min-h-screen bg-[#F2F0E9] text-[#2A2A2A] p-8 font-sans">
      <header className="max-w-7xl mx-auto mb-12 flex justify-between items-end border-b border-[#D1CFCA] pb-6">
        <div className="flex items-center gap-6">
          <div className="w-8 h-10 border border-[#2A2A2A] rounded-t-full flex items-end justify-center pb-1 relative overflow-hidden">
            <div className="w-full h-px bg-[#2A2A2A] absolute bottom-2 opacity-30"></div>
            <div className="w-1 h-1 bg-[#2A2A2A] rounded-full mb-1"></div>
          </div>
      ...
      <main className="max-w-7xl mx-auto grid grid-cols-12 gap-8">
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-8">
              {signals.map((signal) => (
                <article 
                  key={signal.id} 
                  className="border border-[#D1CFCA] p-4 cursor-pointer hover:bg-white/50 transition-colors"
                  onClick={() => setSelectedSignal(signal)}
                >
                  <div className="aspect-[3/4] bg-white border border-[#D1CFCA] mb-4 flex items-center justify-center overflow-hidden">
        ...
        <aside className="col-span-4">
          {selectedSignal ? (
            ...
            <div className="border border-[#D1CFCA] p-8 text-center text-[#8E8C84] font-serif italic">
