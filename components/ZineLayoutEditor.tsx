...
export const ZineLayoutEditor: React.FC<ZineLayoutEditorProps> = ({ page, tone, initialTitle = "Untitled Manifest", onSave, onCancel }) => {
  ...
  return (
    ...
    >
        <div className="absolute top-4 left-4 right-4 z-50 flex justify-between items-center bg-black/50 backdrop-blur-md rounded-2xl px-4 py-2 md:px-6 md:py-3 opacity-0 hover:opacity-100 transition-opacity duration-300 shadow-2xl border border-white/10">
            <div className="flex items-center gap-2 md:gap-6">
                <button onClick={onCancel} className="p-2 md:p-3 text-white/70 hover:text-red-500 rounded-full transition-all"><X size={18} /></button>
            ...
            <div className="flex items-center gap-3 md:gap-4">
                ...
                <div className="hidden md:flex items-center gap-1 border-r border-white/20 pr-3 mr-1">
                    <button onClick={handleSaveDraft} className="p-2 text-white/70 hover:text-emerald-500 transition-colors" title="Save Draft">
                        <Save size={16} />
                    </button>
                ...
                <div className="relative">
                    <button 
                        ...
                        className="px-4 py-1.5 md:py-3 border border-white/20 text-white/70 hover:text-white font-sans text-[7px] md:text-[9px] uppercase tracking-widest font-black rounded-full flex items-center gap-2 transition-all hover:bg-white/10"
                    >
                        {isExporting ? <Loader2 size={12} className="animate-spin" /> : <Download size={12} />}
                        <span className="hidden md:inline">Export</span>
                    </button>
                    {showExportMenu && (
                        <div className="absolute top-full right-0 mt-2 bg-black/80 backdrop-blur-xl border border-white/20 rounded-lg shadow-xl flex flex-col w-40 z-50 overflow-hidden">
                ...
                <button onClick={handleCommit} className={`px-4 md:px-8 py-1.5 md:py-3 ${isSaving ? 'bg-emerald-500 text-white' : 'bg-white text-black'} font-sans text-[7px] md:text-[10px] uppercase tracking-[0.3em] font-black rounded-full shadow-lg flex items-center gap-1.5 md:gap-3 shrink-0 hover:scale-105 active:scale-95 transition-all`}>
        ...
        <div className={`flex-1 flex items-center justify-center overflow-hidden relative transition-all ${isMobile && drawerOpen ? 'pb-[45vh]' : ''}`} onClick={() => setSelectedId(null)}>
             <div ref={containerRef} onClick={(e) => e.stopPropagation()} className="relative shadow-2xl bg-white dark:bg-stone-900 border border-stone-100 dark:border-stone-800 transition-all duration-700" style={{ aspectRatio: `${ratioW}/${ratioH}`, maxHeight: '100vh', maxWidth: '100vw', width: 'auto', height: '100vh' }}>
                         ...
                         {el.type === 'image' && (
                           <>
                             ...
                             {el.aestheticViolation?.isViolation && (
                               <div className="absolute inset-0 border-2 border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.5)] pointer-events-none flex items-start justify-end p-2">
                             ...
                             {el.aestheticViolation?.isViolation && selectedId === el.id && (
                               <motion.button 
                                 ...
                                 className="absolute bottom-2 left-1/2 -translate-x-1/2 bg-stone-900/90 text-white px-3 py-1.5 rounded-full text-[8px] uppercase tracking-widest font-black flex items-center gap-2 backdrop-blur-sm border border-stone-700 hover:bg-stone-800 transition-colors"
                               >
                                 {el.harmonizing ? <Loader2 size={10} className="animate-spin" /> : <Sparkles size={10} className="text-emerald-400" />}
