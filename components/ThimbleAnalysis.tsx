...
export const ThimbleAnalysis: React.FC<Props> = ({ artifactVector, userVector, trajectoryLabel, similarityScore, interpretation, onSave, thumbnailUrl }) => {
  ...
  return (
    <div className="p-6 border border-[#D1CFCA] bg-[#F2F0E9] text-[#2A2A2A] font-sans shadow-sm">
      <div className="flex justify-between items-baseline mb-6 border-b border-[#D1CFCA] pb-4">
      ...
      {thumbnailUrl && (
        <div className="mb-6 aspect-video border border-[#D1CFCA] bg-white flex items-center justify-center">
      ...
      {/* Analysis Points */}
      <div className="space-y-4 text-sm font-serif italic text-[#2A2A2A]">
      ...
      <button 
        onClick={onSave}
        className="mt-8 w-full py-3 bg-[#2A2A2A] text-[#F2F0E9] text-[10px] uppercase tracking-[0.2em] hover:bg-black transition-colors"
      >
