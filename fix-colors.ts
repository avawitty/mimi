...
files.forEach(file => {
  ...
  // Replace `text-white` with `text-nous-text` if it's not inside a specific background that requires white text
  // Actually, a safer approach is to replace `text-white` with `text-nous-text` globally, EXCEPT where the background is explicitly dark.
  // But wait, `bg-nous-base` is white in light mode. So `text-white` on `bg-nous-base` is invisible.
  // Let's replace `text-white` with `text-nous-text` in most places, but we have to be careful.
